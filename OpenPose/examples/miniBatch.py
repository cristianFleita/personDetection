...
class general_mulitpose_model(object):
    ...
    def predict_batch(self, imgfiles):
        start = time.time()
        imgs = []
        ...
        in_blobs = np.concatenate(imgs, axis=0)
        self.pose_net.setInput(in_blobs)
        outputs = self.pose_net.forward()
        print("[INFO]Time Taken in Forward pass: {} ".format(time.time() - start))

        batch_size = len(imgfiles)
        detected_keypoints_batch = [[] for _ in range(batch_size)]
        keypoints_list_batch = [np.zeros((0, 3)) for _ in range(batch_size)]
        res = []
        for b in range(batch_size):
            img = cv2.imread(imgfiles[b])
            height, width, _ = img.shape
            detected_keypoints = []
            ...
            detected_keypoints_batch[b] = detected_keypoints
            keypoints_list_batch[b] = keypoints_list

            valid_pairs, invalid_pairs = self.getValidPairs(outputs, detected_keypoints_batch[b], width,
                                                                             height)
            personwiseKeypoints_batch = self.getPersonwiseKeypoints(valid_pairs, invalid_pairs,
                                                                         keypoints_list_batch[0])
            img_batch = self.vis_pose(img, personwiseKeypoints_batch, keypoints_list_batch[b], )
            FPS = math.ceil(batch_size / (time.time() - start))

            img_batch = cv2.putText(img_batch, "FPS" + str(int(FPS)), (25, 50), cv2.FONT_HERSHEY_SIMPLEX, 1,
                                           (0, 255, 0), 3)
    ...
    def vis_pose(self, img, personwiseKeypoints, keypoints_list):
        img = img.copy()
...
if __name__ == '__main__':
    ...
    img1 = cv2.imread("images/cam1_923.jpg")
    img2 = cv2.imread("images/cam1_923.jpg")
    images = ["images/cam1_923.jpg", "images/cam2_923.jpg"]
    result_images = gmm.predict_batch(images)
    i = 0
